      ...
      bubbles.forEach((bubble, i) => {
        ...
        for (let j = i + 1; j < bubbles.length; j++) {
          ...
          if (dist < minDist && dist > 0) {
            // Collision detected - swap velocities with some dampening
            const nx = dx / dist;
            const ny = dy / dist;
      ...
      <div ref={containerRef} className="relative h-48 overflow-hidden">
        {skills.map((skill, index) => (
          <span
            key={skill.label}
            className={cn(
              "bubble-tag absolute px-2 py-1 rounded-full shadow-sm text-[10px] font-medium whitespace-nowrap transition-none",
              skill.color
            )}
            style={{ 
